...
void main() {
  ...
  group('addEventSnapPoints', () {
    test('adds start and end of each event', () {
      ...
      expect(points.snapPoints, [
        ...
        FloatingDateTime(2024, 1, 1, 11),
        FloatingDateTime(2024, 1, 1, 12),
        FloatingDateTime(2024, 1, 1, 12),
        FloatingDateTime(2024, 1, 1, 13),
      ]);
    });
  });
  ...
  group('addSnapPoint / removeSnapPoint', () {
    ...
    test('remove decreases length and no longer contains the point', () {
      points.addSnapPoint(FloatingDateTime(2024, 1, 1, 14));
      points.removeSnapPoint(FloatingDateTime(2024, 1, 1, 14));
  ...
  group('findSnapPoint', () {
    ...
    test('returns null when list is empty', () {
      ...
      expect(empty.findSnapPoint(FloatingDateTime(2024, 1, 1, 10), snapRange), isNull);
    });

    test('cursor exactly on a snap point returns that point', () {
      final snapPoint = points.findSnapPoint(FloatingDateTime(2024, 1, 1, 10), snapRange);
      expect(snapPoint, FloatingDateTime(2024, 1, 1, 10));
    });
