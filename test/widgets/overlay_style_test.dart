...
// MultiDayOverlayStyle reaches the overlay card, its close button, the barrier and the header date.
void main() {
  ...
  group('MultiDayOverlayStyle.cardTheme', () {
    testWidgets('falls back to the app card theme when unset', (tester) async {
      ...
      final cardTheme = resolvedCardTheme(tester);
      expect(cardTheme.color, const Color(0xFF112233));
      expect(cardTheme.elevation, 6);
    ...
    testWidgets('the theme extension reaches the card', (tester) async {
      ...
      final cardTheme = resolvedCardTheme(tester);
      expect(cardTheme.color, const Color(0xFF00FF00));
      expect(cardTheme.elevation, 8);
    });
  ...
  group('MultiDayOverlayStyle.width and headerHeight', () {
    ...
    // The card's margin and the gap below the header come out of the overlay height first.
  ...
  // A disabled button, filled only for today, like DayHeader, MonthDayHeader and ScheduleDate.
  group('the date in the header', () {
    final dateText = find.descendant(of: find.byKey(MultiDayOverlay.getOverlayCardKey(day)), matching: find.text('15'));

    IconButton dateButton(WidgetTester tester) {
      return tester.widget<IconButton>(find.ancestor(of: dateText, matching: find.byType(IconButton)));
    }
    ...
    testWidgets('uses dateTextStyle', (tester) async {
      ...
      final text = tester.widget<Text>(dateText);
    ...
    testWidgets('is not highlighted when the date is not today', (tester) async {
      ...
      expect(dateText, findsOne, reason: 'the date should still render, just without the highlight');
    });
