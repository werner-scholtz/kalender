...
void main() {
  ...
  group('VerticalDragTarget resizeEvent', () {
    ...
    // A vertical resize must not change the day. The cursor's date follows its
    // horizontal position, so dragging sideways (e.g. from the right edge of a
    // column) used to pull the edge onto the neighbouring day. These check that
    // the edge stays on the event's own day whatever day the cursor is over,
    // only its time of day taking effect.
    testWidgets('bottom resize keeps the end on the event day when the cursor drifts to the previous day',
        (tester) async {
      final ec = DefaultEventsController();
      ec.addEvent(_singleDayEvent(start: eventStart, end: eventEnd));
      final state = await _pumpWeekView(tester, eventsController: ec);
      ...
      final event = ec.events.first;
      // Cursor over Jan 5 (the previous day) at 14:00.
      final result = state.resizeEvent(event, ResizeDirection.bottom, InternalDateTime(2025, 1, 5, 14, 0));

      expect(result, isNotNull);
      expect(
        result.dateTimeRange.start.isAtSameMomentAs(eventStart),
        isTrue,
        reason: 'start is untouched by a bottom resize',
      );
      expect(
        result.dateTimeRange.end.isAtSameMomentAs(DateTime(2025, 1, 6, 14, 0)),
        isTrue,
        reason: 'end stays on Jan 6 at 14:00, not jumping to Jan 5',
      );
    });

    testWidgets('top resize keeps the start on the event day when the cursor drifts to the previous day',
        (tester) async {
      final ec = DefaultEventsController();
      ec.addEvent(_singleDayEvent(start: eventStart, end: eventEnd));
      final state = await _pumpWeekView(tester, eventsController: ec);
      ...
      final event = ec.events.first;
      // Cursor over Jan 5 (the previous day) at 11:00.
      final result = state.resizeEvent(event, ResizeDirection.top, InternalDateTime(2025, 1, 5, 11, 0));

      expect(result, isNotNull);
      expect(result.dateTimeRange.end.isAtSameMomentAs(eventEnd), isTrue, reason: 'end is untouched by a top resize');
      expect(
        result.dateTimeRange.start.isAtSameMomentAs(DateTime(2025, 1, 6, 11, 0)),
        isTrue,
        reason: 'start stays on Jan 6 at 11:00, not jumping to Jan 5',
      );
    });

    testWidgets('bottom resize keeps the end on the event day when the cursor drifts to the next day', (tester) async {
      final ec = DefaultEventsController();
      ec.addEvent(_singleDayEvent(start: eventStart, end: eventEnd));
      final state = await _pumpWeekView(tester, eventsController: ec);

      final event = ec.events.first;
      // Cursor over Jan 7 (the next day) at 14:00.
      final result = state.resizeEvent(event, ResizeDirection.bottom, InternalDateTime(2025, 1, 7, 14, 0));

      expect(result, isNotNull);
      expect(
        result.dateTimeRange.end.isAtSameMomentAs(DateTime(2025, 1, 6, 14, 0)),
        isTrue,
        reason: 'end stays on Jan 6 at 14:00, not jumping to Jan 7',
      );
    });
  });
