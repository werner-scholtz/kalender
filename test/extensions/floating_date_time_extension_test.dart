...
void main() {
  ...
  group('FloatingDateTime', () {
    group('fromDateTime', () {
      test('creates an FloatingDateTime from a local DateTime preserving components', () {
      ...
      test('creates an FloatingDateTime from a UTC DateTime preserving components', () {
      ...
      test('creates an FloatingDateTime from a TZDateTime with negative UTC offset', () {
      ...
      test('creates an FloatingDateTime from a TZDateTime with positive UTC offset', () {
    ...
    group('isToday with explicit now', () {
      test('returns true when now matches the same day', () {
        final date = FloatingDateTime(2026, 4, 13);
      ...
      test('returns true regardless of time-of-day component', () {
        final date = FloatingDateTime(2026, 4, 13, 23, 59, 59);
      ...
      test('falls back to location-based behavior when now is null', () {
        final now = DateTime.now();
        final date = FloatingDateTime(now.year, now.month, now.day);
    ...
    group('isSameDay', () {
      test('returns true for the same date', () {
        final date1 = FloatingDateTime(2024, 1, 15, 10, 30);
        final date2 = FloatingDateTime(2024, 1, 15, 22, 0);

        expect(date1.isSameDay(date2), true);
      });

      test('returns false for different days', () {
        final date1 = FloatingDateTime(2024, 1, 15);
        final date2 = FloatingDateTime(2024, 1, 16);
      ...
      test('returns false for different months', () {
        final date1 = FloatingDateTime(2024, 1, 15);
        final date2 = FloatingDateTime(2024, 2, 15);
      ...
      test('returns false for different years', () {
        final date1 = FloatingDateTime(2024, 1, 15);
        final date2 = FloatingDateTime(2025, 1, 15);

        expect(date1.isSameDay(date2), false);
      });

      test('returns true when both are at start of day', () {
        final date1 = FloatingDateTime(2024, 6, 1);
        final date2 = FloatingDateTime(2024, 6, 1);

        expect(date1.isSameDay(date2), true);
      });

      test('returns true when one is at start and other at end of day', () {
        final startOfDay = FloatingDateTime(2024, 3, 10, 0, 0, 0);
        final endOfDay = FloatingDateTime(2024, 3, 10, 23, 59, 59);
      ...
      test('returns false for adjacent days at midnight boundary', () {
        final endOfDay = FloatingDateTime(2024, 3, 10, 23, 59, 59);
        final startOfNextDay = FloatingDateTime(2024, 3, 11, 0, 0, 0);
      ...
      test('works correctly with dates created from TZDateTime', () {
        ...
        final nyDate = FloatingDateTime.fromDateTime(TZDateTime(nyLocation, 2024, 1, 15, 22, 0));
        final tokyoDate = FloatingDateTime.fromDateTime(TZDateTime(tokyoLocation, 2024, 1, 15, 12, 0));
      ...
      test('returns false for TZDateTimes that are same UTC instant but different display days', () {
        ...
        final nyDate = FloatingDateTime.fromDateTime(TZDateTime(nyLocation, 2024, 1, 15, 23, 0));
        final tokyoDate = FloatingDateTime.fromDateTime(TZDateTime(tokyoLocation, 2024, 1, 16, 13, 0));

        expect(nyDate.isSameDay(tokyoDate), false);
      });

      test('is symmetric', () {
        final date1 = FloatingDateTime(2024, 5, 20, 8, 0);
        final date2 = FloatingDateTime(2024, 5, 20, 18, 0);
    ...
    group('type checks', () {
      test('is a subclass of DateTime', () {
        final internal = FloatingDateTime(2024);
        expect(internal, isA<DateTime>());
      });

      test('is recognized as FloatingDateTime via is check', () {
        final DateTime date = FloatingDateTime(2024);
        expect(date is FloatingDateTime, true);
      });
    });
  });
}
