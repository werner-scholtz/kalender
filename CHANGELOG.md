...
- `ResizeHandlePositioner` takes a `BuildContext` as its first argument, and `ResizeHandles.builder` is replaced by `TileComponents.buildResizeHandles`. The positioner keeps its other parameters, including the `TileComponents` the `ResizeHandles` base class requires.
