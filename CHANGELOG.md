...
- View configurations built with the same arguments now compare equal, so a rebuild no longer recreates the view controller. `PageIndexCalculator` and its seven subclasses had no `==`, and both view configurations compare it, so two identical configurations were never equal. A calendar whose configuration is built inside `build` therefore threw away its event layout and multi-day frame caches on every rebuild. Scroll and zoom were unaffected, since the transition policy restores them. `ScheduleViewConfiguration` had no `==` at all and is now compared too. [#404](https://github.com/werner-scholtz/kalender/pull/404)
- `ViewConfiguration.nowCallback` takes part in `==`, so changing it updates the time indicator and the today highlight. It is read from the view controller's configuration, which is fixed when the controller is created, so a change has to recreate it. Pass a top-level or static function. The date, scroll and zoom resolvers are deliberately not compared: they are read from the incoming configuration at a view switch, so they are always current. [#404](https://github.com/werner-scholtz/kalender/pull/404)
- `CalendarView` reacts to a replaced `calendarController`, detaching from the old one and attaching to the new. Previously the view stayed attached to the old controller. [#404](https://github.com/werner-scholtz/kalender/pull/404)
- `MonthViewConfiguration` compares `name`, which `restorePerView` matches on. [#404](https://github.com/werner-scholtz/kalender/pull/404)
